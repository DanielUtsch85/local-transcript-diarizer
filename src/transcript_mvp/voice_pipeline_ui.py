...
def _run_pipeline_from_ui(
    *,
    ...
    with st.status("Voice-Pipeline laeuft...", expanded=True) as status:
        ...
        selected = filter_segments(load_segments(rows_for_prepared), speaker, min_duration_sec, max_duration_sec)
        extracted = []
        ...
        scored = [score_segment(Path(row["file"]), speaker=speaker, quality_config=cfg["quality"]) for row in extracted]
        ...
        if run_synthesis:
            ...
            write_synthesis_log(manifests_dir / "synthesis_runs.jsonl", metadata, result)
